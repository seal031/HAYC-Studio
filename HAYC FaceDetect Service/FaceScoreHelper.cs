...
public class FaceScoreHelper
{
    ...
    private static DirectoryInfo parentPath = System.IO.Directory.GetParent(Application.StartupPath);
    ...
    {
        using (FileStream fs = new FileStream(parentPath.FullName + "\\DataFile.faceinfo", FileMode.OpenOrCreate))
        {
    ...
    {
        LogHelper.WriteLog("服务根目录为"+parentPath.FullName);
        ...
        using (FileStream fs = new FileStream(parentPath.FullName + "\\DataFile.faceinfo", FileMode.OpenOrCreate))
        {
            ...
            {
                LogHelper.WriteLog("反序列化失败。"+ex.Message);
            }
        }
        return list;
    }
